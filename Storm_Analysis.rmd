...
```{r, cache=TRUE}
data <- data %>% filter(PROPDMGEXP %in% c(letters,LETTERS)|CROPDMGEXP %in% c(letters,LETTERS))

#We will still have a few of the invalid characters left
#Where they correspond ot valid letters on the other column.
data %>% group_by(PROPDMGEXP) %>% summarize(count=n()) 
data %>% group_by(CROPDMGEXP) %>% summarize(count=n())
```

Finally, I'll write a simple function to transform the CROPDMG and PROPDMG variables and apply it with mutate():
```{r, cache=TRUE}
